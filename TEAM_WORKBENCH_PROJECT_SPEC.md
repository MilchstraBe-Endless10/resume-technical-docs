...
- DSH Codex Subagent 兼容路径（Hybrid Lite 可选）；
- Tool / command / runtime lifecycle 映射到统一 `WorkbenchEvent`；
- Cancel 当前 Run；
...
**决定：** Adapter 仅接收 RuntimeRunRequest、Context Package 与授权 ResourceRefs；Workbench Data Plane 保持数据边界。
**状态：** Accepted
...
Execution Backend / Harness
= 用哪套 Agent Loop / CLI / Runtime 执行
...
interface SearchIndexProvider {
  probe(): Promise<SearchCapabilities>
...
text / page / image / frame / clip / metadata
      ↓
